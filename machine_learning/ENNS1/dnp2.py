...
class NeuralNet(nn.Module):
    ...
    """
    def __init__(self, input_size: int, output_size: int, hidden_size, regression=False):
        """
...
class DeepNet:
    ...
    """
    def __init__(self, max_feature: int, num_classes: int = 2, hidden_size: list = [64, 32],
                 q: int = 2, num_dropout: int = 10, regression: bool = False, pre_feature: list = [], prob=0.5):
        """
        initialization function
    ...
    def set_parameters(self, x: torch.Tensor, batch_size: int, learning_rate: float, epochs: int,
        ...
        if dropout_prop is None:
            self.dropout_prop = [self.prob] * len(self.hidden_size)
        else:
    ...
    def update_nn_weight(self, x: torch.Tensor, y: torch.Tensor, verbosity: int = 0):
        ...
        self.nnmodel = NeuralNet(input_size, output_size, self.hidden_size)
        
        # to enforce reproducibility
        # uncomment the following two lines
        # Chen added on Oct 23
        torch.manual_seed(42)
        np.random.seed(42)
    ...
    def dropout(self) -> nn.Module:
        ...
        for h in range(min(len(self.hidden_size) - 1, len(self.dropout_prop))):
            ...
            model_dp.hiddens[h].weight.data[:, dropout_index] = torch.zeros(
    ...
    def compute_input_gradient(self, x: torch.Tensor, y: torch.Tensor, model: nn.Module) -> torch.Tensor:
        ...
        """
        model_gr = NeuralNet(self.p, self.num_classes, self.hidden_size)
        ...
        if len(y.shape) > 1:
            y = y.squeeze(1)
        if self.regression:
            output_gr = output_gr.squeeze(1).float()
            y = y.float()
        loss_gr = self.criterion(output_gr, y)
        loss_gr.backward()
    ...
    def train(self, x: torch.Tensor, y: torch.Tensor, batch_size: int = 16, learning_rate: float = 0.01,
    ...
    def train_return_next(self, x: torch.Tensor, y: torch.Tensor, batch_size: int = 16, learning_rate: float = 0.01,
                         epochs: int = 50, dropout_prop: list = None, val: list = None, verbosity: int = 0,
